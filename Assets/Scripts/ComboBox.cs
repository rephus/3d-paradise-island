...
public class ComboBox
{
	...
	public ComboBox( Rect rect, int selectedIndex, string[] stringContent ){
		...
		GUIContent[] comboBoxList = new GUIContent[stringContent.Length];
		
		for(int i=0; i < stringContent.Length; i++) {
			comboBoxList[i] = new GUIContent(stringContent[i]);
		}

		Setup(rect, comboBoxList[selectedIndex], comboBoxList,"button", "box", listStyle);
	}

    public ComboBox( Rect rect, GUIContent buttonContent, GUIContent[] listContent, GUIStyle listStyle ){
		Setup(rect, buttonContent, listContent, "button", "box", listStyle);
    }
	...
	public ComboBox(Rect rect, GUIContent buttonContent, GUIContent[] listContent, string buttonStyle, string boxStyle, GUIStyle listStyle){
		Setup(rect, buttonContent, listContent, buttonStyle,boxStyle, listStyle);
	}

	public void Setup(Rect rect, GUIContent buttonContent, GUIContent[] listContent, string buttonStyle, string boxStyle, GUIStyle listStyle){
		this.rect = rect;
